  ...
  if (jobs && Array.isArray(jobs) && jobs.length > 0) {
    ...
    for (let i = 0; i < jobs.length; i++) {
      ...
      if (jobs[i]['job-status-' + jobKey]) {
        ...
        jobs[i].pidSet = false;
        let jobsChild = spawn('node', [path.resolve(projectpath, jobs[i]['job-path-' + jobKey])], {
  ...
  doPreChecks({ UID, port, projectpath, name, pkginstallopt }, (err) => {
    if (err) {
      ...
      if (projectpath && projectpath.length > 0 && startfilepath && startfilepath.length > 0) {
        ...
        let child = spawn('node', [path.resolve(projectpath, startfilepath)], {
          env: envvariables, detached: true, cwd: projectpath
        });
        let err = null;
        let success = null;
        ...
        child.stderr.on('data', (data) => {
          ...
          let callBackTimer = setInterval(async function () {
            if (success && callbackSent === 0) {
              ...
              handleJobs(projectpath, jobs, envvariables, UID);
            } else if (callBackCount > 4 && callbackSent === 0 && err) {
              clearTimeout(callBackTimer);
              callbackSent++;
              callback(err);
              callbackType = 'error';
...
const _stopProject = async (pid) => {
  let { UID, jobPids } = await _projectFieldByKey('pid', pid, ['UID', 'jobPids']);
  process.kill(pid);
  if (activeUID[UID]) {
