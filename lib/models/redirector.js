...
const _redirect = (redirectionPath, res, token) => {
  ...
  if (token) {
    htmlContent = htmlContent.replace('let authToken = null;', `let authToken = "${token}";`);
  }
...
const _redirectToAuthentication = (compileParams = {}, res) => {
  const indexHTMLPath = path.join(__dirname, '../view', 'login.ejs');
  const htmlTemplate = fs.readFileSync(indexHTMLPath, 'utf8');
  const compiled = ejs.compile(htmlTemplate);
  const { protocol } = getSSLConfig();
  compileParams.PROTOCOL = protocol;
  let htmlContent = compiled(compileParams);
  htmlContent = htmlContent.split('${PORT}').join(process.env.PORT.trim());
  htmlContent = htmlContent.replace('id="redirect-path"', 'id="redirect-path" value="' + compileParams.redirectPath + '"');
  res.set('Content-Type', 'text/html');
  res.send(Buffer.from(htmlContent));
};
