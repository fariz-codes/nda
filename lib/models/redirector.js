...
const _redirectToEditConfig = (compileParams = {}, res) => {
  ...
  if (compileParams.err && compileParams.err.length > 0) {
    ...
    htmlContent = htmlContent.replace('id="submit-button" onclick="enableLoader(true);', 'id="submit-button" disabled="true" style="background-color: grey !important; color: white !important;"');
  ...
  if (compileParams.sslKeyErr !== '' || compileParams.sslCertErr !== '') {
    if (compileParams.sslCertErr) {
      htmlContent = htmlContent.replace('class="cert-file-err"', 'class="cert-file-err" style="visibility: visible;"');
    }
    if (compileParams.sslKeyErr) {
      htmlContent = htmlContent.replace('class="key-file-err"', 'class="key-file-err" style="visibility: visible;"');
    }
  }
