...
const { getAuthSetupStatus, getSessionExpiry } = require('../helpers/utilities');
...
const _redirectToAddProject = (compileParams = {}, res) => {
  const sessionExpiry = getSessionExpiry(res.decoded);
  ...
  compileParams.sessionExpiry = sessionExpiry.minutes + ':' + sessionExpiry.seconds;
...
const _redirectToEditConfig = (compileParams = {}, res) => {
  ...
  let scriptErr = compileParams.err.toString();
  if (scriptErr.indexOf('must be privileged') > -1) {
    compileParams.err = 'Please re-start the NDA as a `sudo` user to enable the start-on-boot';
  }
  if (scriptErr.indexOf('Operation not permitted') > -1) {
    compileParams.err = 'Permission to modify the start-on-boot is rejected. Please try again after sometime or re-start the NDA as a `sudo` user';
  }
  compileParams.sessionExpiry = sessionExpiry.minutes + ':' + sessionExpiry.seconds;
  ...
  if (os.type().indexOf('Darwin') > -1) {
    ...
    htmlContent = htmlContent.replace('class="mac-boot-start-msg"', 'class="" ');
  }
...
const _redirectToAuthErr = (compileParams = {}, res) => {
  const indexHTMLPath = path.join(__dirname, '../view', 'auth.ejs');
  const htmlTemplate = fs.readFileSync(indexHTMLPath, 'utf8');
  const compiled = ejs.compile(htmlTemplate);
  const { protocol } = getSSLConfig();
  if (!compileParams.successMsg) compileParams.successMsg = "";
  if (!compileParams.errMsg) compileParams.errMsg = "";

  compileParams.PROTOCOL = protocol;
  let htmlContent = compiled(compileParams);
  htmlContent = htmlContent.split('${PORT}').join(process.env.PORT.trim());
  let authSetupStatus = getAuthSetupStatus();
  if (!authSetupStatus.status) {
    htmlContent = htmlContent.replace('id="ndalogindiv"', 'id="ndalogindiv" style="display:none;"');
  } else {
    htmlContent = htmlContent.replace('id="ndaautherr"', 'id="ndaautherr" style="display:none;"');
  }
...
module.exports = {
  ...
  _redirectToLogsList, _redirectToEditConfig, _redirectToAuthErr
};