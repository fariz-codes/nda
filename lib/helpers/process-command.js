...
async function handleCommands(isLinux) {
  switch (true) {
    case process.argv.slice(2)[0].toString().trim() === 'info-auth':
      let authSetup = utils.getAuthSetupStatus();
      if (authSetup.status && authSetup.path) {
        console.log(`Your qr code for authentication is saved as ${authSetup.path}\nPlease extract it using the provided password and configure it in your authenticator app.`);
      } else {
        console.log("Authentication not initialized. Please run `nda setup-auth [password]` to initiate it.");
      }
      break;
    case process.argv.slice(2)[0].toString().trim() === 'setup-auth':
      if (utils.isAdmin()) {
        let password = process.argv.slice(3)[0];
        if(password && password.toString().trim().length > 0) {
          utils.generateQRCode(password);
        } else {
          console.log(ERR_TEXT + "Password is required to generate the qr code image");
        }
      } else {
        const platform = os.platform();
        let accessMessage = platform === 'win32' ? ' as administrator' : ' with sudo access';
        console.log(ERR_TEXT + "This command requires admin privileges. Please restart the terminal" + accessMessage);
      }
      break;
