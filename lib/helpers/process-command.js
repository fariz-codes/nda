...
const { stopAllProjects, _projects, _startProject } = require('../models/process');
const os = require('os');
const fs = require('fs');
...
async function handleCommands(isLinux) {
  switch (true) {
    ...
    case process.argv.slice(2)[0].toString().trim() === 'status':
      ...
      break;
    case process.argv.slice(2)[0].toString().trim() === 'refresh':
      ...
      if (previousPidId && utils.isPidRunning(previousPidId)) {
        ...
        console.log('nda is not running. Refresh option will work only when nda is in running state');
      }
      break;
  };
}
