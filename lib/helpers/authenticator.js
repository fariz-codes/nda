...
const verifyAuthInfo = (username, password) => {
  ...
  if (authInfo && authInfo.key && authInfo.iv && authInfo.username && authInfo.password) {
    let encryptedUsername = encryptString(Buffer.from(authInfo.key), Buffer.from(authInfo.iv), username);
    let encryptedPassword = encryptString(Buffer.from(authInfo.key), Buffer.from(authInfo.iv), password);
    if (authInfo.username === encryptedUsername && authInfo.password === encryptedPassword) {
