...
const getAuthStatusText = (path, filename) => {
  return "\nQR code image is saved at '" + path + "' with filename: '" + filename + ".zip'\nPlease extract it using the provided password and configure it in your authenticator app."
};
...
const generateQRCode = (password) => {
  ...
  qrcode.toFile(qrCodeImgFilePath, otpauthURL, async (err) => {
    ...
    output.on('close', () => {
      ...
      console.log(getAuthStatusText(CHILD_PROCESS_BASE_CONFIG_PATH, authQrCodeFile));
    });
...
const getAuthSetupStatus = () => {
  ...
  return {
    status: authQRCode, path: CHILD_PROCESS_BASE_CONFIG_PATH, fileName: authQrCodeFile + '.zip'
  };
};

const getSessionExpiry = (decoded) => {
  let minutes = 0;
  let seconds = 0
...
module.exports = {
  ...
  formatBytes, getAllowedFields, generateQRCode, isAdmin, verifyOtp, getAuthSetupStatus, getAuthStatusText, getSessionExpiry
};