...
const isAdminUser = () => {
  let isAdmin = true;
  let adminCmd = isLinux ? 'whoami' : 'net session';
  try {
    let execResponse = execSync(adminCmd);
    execResponse = execResponse ? convertBufferToArray(execResponse).toString().trim().toLowerCase() : '';
    if (isLinux) {
      isAdmin = execResponse && execResponse.indexOf('root') > -1 ? true : false;
    } else if (execResponse && execResponse.indexOf(AUTHENTICATION.Error.accessDenied) > -1) {
      isAdmin = false;
    }
  } catch (err) {
    isAdmin = false;
