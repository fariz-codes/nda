...
const isPortInUse = (portNumber, callback) => {
  ...
  let findPortCmd = osType.indexOf('Windows') > -1 ? 'netstat.exe' : osType.indexOf('Linux') > -1 ? 'netstat' : 'ps';
  let findPortOptions = osType.indexOf('Windows') > -1 ? ['-na'] : osType.indexOf('Linux') > -1 ? ['-lptn'] : ['aux'];
  let pingCmd = spawn(findPortCmd, findPortOptions);
  let listeningPorts = '';
