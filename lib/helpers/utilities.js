...
const getPortCmdByOS = (osType) => {
  ...
  } else if (osType.toString().toLowerCase().indexOf('darwin') > -1) {
...
const isPortInUse = (portNumber, callback) => {
  ...
  if (cmd && options) {
    ...
    pingCmd.on('exit', function () {
      if (error) {
        callback({ inUse: false });
      } else if ((osType.indexOf('Windows') > -1 && listeningPorts && listeningPorts.indexOf(`0.0.0.0:${portNumber}`) > -1) || (osType.indexOf('Windows') < 0 && listeningPorts.indexOf(portNumber) > -1)) {
        callback({ inUse: true });
      } else {
        callback({ inUse: false });
      }
...
const isPidRunning = (pid) => {
  try {
    const osType = os.type();
    if (osType.toString().toLowerCase().indexOf('darwin') > -1) {
      return true;
    }
