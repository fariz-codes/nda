...
const { authAppName, authUserName, AUTH_QR_CODE_IMG_PATH, authQrCodeFile } = require('../../config/project-configs');
...
const generateQRCode = (password) => {
  ...
  const qrCodeImgFilePath = path.join(AUTH_QR_CODE_IMG_PATH, authQrCodeFile + '.png');
  const qrCodeZipFilePath = path.join(AUTH_QR_CODE_IMG_PATH, authQrCodeFile + '.zip');
  ...
  qrcode.toFile(qrCodeImgFilePath, otpauthURL, async (err) => {
    ...
    output.on('close', () => {
      ...
      execSync(`icacls "${qrCodeZipFilePath}" /inheritance:r /remove:g *S-1-1-0 /grant:r ${os.userInfo().username}:(F)`, { stdio: 'inherit' });
      console.log("QR code image is saved at " + qrCodeZipFilePath + ". Please configure it in your authenticator app.");
    });
...
const getAuthSetupStatus = () => {
  let authQRCode = fs.existsSync(AUTH_QR_CODE_IMG_PATH);
  return { status: authQRCode ? true : false, path: AUTH_QR_CODE_IMG_PATH };
};
