...
const _addForm = async (req, res) => {
  try {
    let compilationParams = { errMsg: '', successMsg: '', };
...
const _store = async (req, res) => {
  try {
    ...
    if (errMsg === '') {
      ...
      _redirectToAddProject({ successMsg: '', errMsg, fieldId, fields: newFields }, res);
    }
  } catch (error) {
    writeLog(null, 500, error, '_2');
  }
...
const _clone = async (req, res) => {
  try {
    ...
    _redirectToAddProject({ successMsg: PROJECT.Success['#220'], errMsg: '', fields: projectInfo }, res);
  } catch (error) {
    writeLog(null, 500, error, '_2');
  }
};
