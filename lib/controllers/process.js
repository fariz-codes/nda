...
const { _projectFields, _startProject, _hashFromName, _projects, _updateProjects, _stopProject, _deleteProject, stopAllProjects, _projectJobs, _cloneProject } = require('../models/process');
...
const _addForm = async (req, res) => {
  try {
    let compilationParams = { errMsg: '', successMsg: '', clonedUID: '' };
...
const _store = async (req, res) => {
  try {
    ...
    if (errMsg === '') {
      ...
      _redirectToAddProject({ clonedUID: '', successMsg: '', errMsg, fieldId, fields: newFields }, res);
    }
  } catch (error) {
    writeLog(null, 500, error, '_2');
  }
...
const _clone = async (req, res) => {
  try {
    let projectInfo = await _cloneProject(req.query.UID);
    _redirectToAddProject({ clonedUID: projectInfo.UID, successMsg: PROJECT.Success['#220'], errMsg: '', fields: projectInfo }, res);
  } catch (error) {
    writeLog(null, 500, error, '_2');
  }
};
